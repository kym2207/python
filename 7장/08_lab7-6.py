...
max_pop = 0 #최대 인구를 구한기 위한 최소값
min_pop = 999999999999999999 #최소 인구를 구한기 위한 최대값
total_pop = 0 #모든인구 구하기위한 변수
max_city = None #최대 도시 이름을 저장할 빈 변수
min_city = None #최소 도시 이름을 저장할 빈 변수
for city in city_info: #city에 city_info에 있는 값(ex ('서울',9765))를 하나하나 넣으면서 반복
    total_pop += city[1] #모든 도시인구의 합 구하기
    if city[1] > max_pop : #가장 인구가 많은 도시 구하기
        max_pop = city[1]
        max_city = city
    if city[1] < min_pop : #가장 인구가 적은 도시 구하기
        min_pop = city[1]
        min_city = city
